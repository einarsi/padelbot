...
async def _get_practices(
    ...
    events = (
        await s.get_events(group_id=group_id, min_start=min_start, max_start=max_start)
        or []
    )
    retval = []
    for event in events:
...
async def get_last_practice_in_series(
    s: spond.Spond, group_id: str, event: dict
) -> dict | None:
    ...
    for previous_event in events:
        ...
        if (
            abs((start_time - previous_start_time).total_seconds() - 7 * 24 * 60 * 60)
            <= 5 * 60
        ):
...
async def quarantine_players_from_last_event(
    s: spond.Spond, group_id: str, event: dict, quarantine_days: int = 1
) -> datetime | None:
    ...
    while True:
        ...
        if upcoming_events:
            for event in reversed(upcoming_events):
                ...
                quarantine_end_time = await quarantine_players_from_last_event(
                    s, group_id, event
                )
